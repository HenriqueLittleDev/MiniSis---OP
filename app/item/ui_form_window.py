...
class ItemFormWindow(QWidget):
    def __init__(self, item_id=None):
        ...
        self.search_supplier_button.clicked.connect(self._open_supplier_search)

        self._selected_supplier_id = None
        self.supplier_search_window = None


    def _open_supplier_search(self):
        from app.supplier.ui_search_window import SupplierSearchWindow
        if self.supplier_search_window is None:
            self.supplier_search_window = SupplierSearchWindow(selection_mode=True)
            self.supplier_search_window.supplier_selected.connect(self._set_selected_supplier)
    ...
    def _set_selected_supplier(self, supplier_data):
        """Recebe o fornecedor selecionado e atualiza a UI."""
        self._selected_supplier_id = supplier_data['ID']
    ...
    def load_item_data(self):
        if self.current_item_id:
            ...
            if response["success"]:
                ...
                self._selected_supplier_id = item.get('ID_FORNECEDOR_PADRAO')
                if self._selected_supplier_id:
                    from app.supplier.service import SupplierService
                    supplier_service = SupplierService()
                    sup_response = supplier_service.get_supplier_by_id(self._selected_supplier_id)
                    if sup_response["success"]:
                        supplier = sup_response["data"]
